	...
	function store()
	{
		...
		switch ($data['typefield'])
		{
			case 'text':
				...
				$defaultvalue = 't_initvalueT==='.$t_initvalueT.'[--]t_maxchar==='.$t_maxchar.'[--]t_texttype==='.$t_texttype.'[--]t_minchar==='.$t_minchar;
				break;

			case 'hidden':
				$t_initvalueH = $data['t_initvalueH'];
				$defaultvalue = 't_initvalueH==='.$t_initvalueH;
				break;
			...
			case 'radiobutton':
				...
				$defaultvalue = 't_listHRB==='.$t_listHRB;
				break;
		...
		if (count($forms ) > 0)
		{
			...
			if ($rowTable->saveresult == 1) {
				...
				{
					$query = "ALTER TABLE ".$tn." ADD ".$row->name." TEXT NULL";
			...
			foreach($cids as $cid) {
				...
				if (!$row->delete( $cid )) {
					...
					if ($saveDB == true)
					{
						...
						if (!isset( $tableFields[$tn][$row->name] )) 
						{
							$query = "ALTER TABLE ".$tn." DROP ".$field->name;
							if (!$dba->Execute($query)) 
							{	
								echo JText::_( 'Problem with' )." (".$query.")";
							}
