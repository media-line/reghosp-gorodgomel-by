...
class AiContactSafeModelFields extends AiContactSafeModelDefault {
	...
	function checkBeforeWrite($postData) {
		...
		if (strlen(trim($postData['name'])) == 0) {
			...
			$postData['name'] = str_replace(' ','_',$postData['name']);
			if ( substr($postData['name'],0,5) != 'aics_' && $postData['name'] != 'name' && $postData['name'] != 'email' && $postData['name'] != 'subject' && $postData['name'] != 'send_to_sender'  ) {
		...
		if (substr(trim($postData['field_values']),-1,1) == ';') {
		...
		$postData['label_after_field'] = $postData['label_after_field']?1:0;
		$postData['send_message'] = $postData['send_message']?1:0;
		$postData['field_required'] = $postData['field_required']?1:0;
		$postData['field_in_message'] = $postData['field_in_message']?1:0;
