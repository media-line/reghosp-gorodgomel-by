...
class plgContentMavikThumbnails extends JPlugin
{
	...
	*/
	function plgContentMavikThumbnails( &$subject, $params )
	{
		...
		if($this->jVersion == '1.5') {
			...
			$this->url = JURI::base().'plugins/content';
		} else {
			// 1.6
			$this->path = JPATH_PLUGINS.DS.'content'.DS.'mavikthumbnails';
			$this->url = JURI::base().'plugins/content/mavikthumbnails';
		}
	...
	*/
	 ...
	 * что необходимо для обработки блогов
	 *
	 * @param	string		The context for the content passed to the plugin.
	 * @param	object		The content object.  Note $article->text is also available
	 * @param	object		The content params
...
class plgContentMavikThumbnailsProportions
{
	...
	/**
	 * Возвращате координаты и размер используемой в оригинальном ибображении области
	 * @return array
	 */
	function getArea()
	{
